...
* **RAG & AI Orchestration**: [LangChain](https://www.langchain.com/) / [LlamaIndex](https://www.llamainced.ai/) 
...
```bash
   git clone [https://github.com/mujeebmasi/video-assistant-rag.git](https://github.com/mujeebmasi/video-assistant-rag.git)
...
Ensure your PostgreSQL instance is running and has the pgvector extension active. Run your migrations or database setup scripts:
...
Accepts a video file or external URL, extracts transcripts, chunks the text, and commits the generated vector embeddings to PostgreSQL.

POST /api/v1/query

Takes a user query, performs a similarity search over the video chunks via pgvector, injects the relevant context into the prompt template, and returns the LLM-generated answer alongside source timestamps.
